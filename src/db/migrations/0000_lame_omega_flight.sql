...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'premiumKeys_activatedBy_users_isu_id_fk') THEN
        ALTER TABLE "premiumKeys" ADD CONSTRAINT "premiumKeys_activatedBy_users_isu_id_fk" FOREIGN KEY ("activatedBy") REFERENCES "public"."users"("isu_id") ON DELETE no action ON UPDATE no action;
    END IF;
END $$;
--> statement-breakpoint
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'userLessons_isu_id_users_isu_id_fk') THEN
